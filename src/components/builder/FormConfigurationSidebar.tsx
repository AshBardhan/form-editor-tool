 ...
 */
const FormConfigurationSidebar = memo(
  function FormConfigurationSidebar(): JSX.Element {
    ...
    const selected = formFields.find((f) => f.id === selectedFieldId);
    ...
    const validateProps = () => {
      ...
      if (!result.success) {
        ...
        const combined = Object.entries(fieldErrors).flatMap(
          ([, msgs]) => msgs,
        );
